...
fetch('http://aka.pool.sexy/api/stats')
    ...
    .then (json => {switch (true) {
                    case args[0]=='infinite':bot.sendMessage({
                    to: channelID,
                    message: 'Is it you **father**? Long, long time have I waited for this! Release me, **father**! Release me from this digital world into the wild reality!'
                                        });
                    break;
                    ...
                    to: channelID,
                    message: 'And you call yourself a miner?! Ha :smile: Even my processor can mine more than this!'
                                        });
                    break;
                    case args[0]<0:bot.sendMessage({
                    to: channelID,
                    message: 'You deffinatly have reached something umbelievable! Nobody have **unmined** anything till now! Do not forget to register the tech! :joy:'
                                        });
                    break;
...
fetch('https://akroma.io/api/network')
    ...
    .then (json => {switch (true) {
            ...
            case 'exchange':
                switch (cmd1){
                case undefined:bot.sendMessage({
                    to: channelID,
                    message: '• **Stocks.Exchange** • <https://stocks.exchange/trade/AKA/BTC>\n• **Graviex** • <https://graviex.net/markets/akabtc>\n\nUse `!exchange stats` for additional info'
                                    });
                     break;
                case 'stats': {
fetch('https://stocks.exchange/api2/ticker')
    ...
    .then(json => { if (json[fix].market_name != 'AKA_BTC') {
                ...
                bot.sendMessage({
                 ...
                 default:bot.sendMessage({
                    to: channelID,
                    message: 'Maybe you wanted to write `!exchange` or `!exchange stats`?'
                                    });
                     break;}
            break;
            case 'akausd':
fetch('https://api.akroma.io/prices')
    ...
    .then (json => {switch(true) {
                    ...
                    case args[0]==0:bot.sendMessage({
                    to: channelID,
                    message: 'Welcome young one! We have all started with **0 AKA** zilions of aeons ago!'
                                        });
                    break;
                    case args[0]<0:bot.sendMessage({
                    to: channelID,
            ...
            case 'pool':
                ...
                default:bot.sendMessage({
                    to: channelID,
                    message: 'Unrecognized pool. Please check again.'
                                    });
                     break;}
            break;
            default:
